...
from ..models import User, PlanTier, Transaction, POSITIVE_TRANSACTION_TYPES
...
def get_limits(db: Session = Depends(get_db), uid: int = Depends(get_current_user_id)):
    ...
    if user.plan_tier == "PREMIUM":
        ...
        tracked_count = (
            db.execute(
                select(func.count(func.distinct(Transaction.card_id)))
                .where(Transaction.user_id == uid)
                .where(Transaction.type.in_(tuple(POSITIVE_TRANSACTION_TYPES)))
            ).scalar()
            or 0
        )
